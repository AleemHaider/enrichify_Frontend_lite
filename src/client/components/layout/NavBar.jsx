...
import { logoutClient } from '../../../redux/client-redux/auth/action';
import { getClientUser } from '../../../redux/client-redux/dashboard/action';
...
const NavBar = () => {
  ...
  // const userData = useSelector((state) => state.userDataReducer?.loading)
  const userData = useSelector(state => state.dashboardReducer.getUser.data?.data)
  const loading = useSelector(state => state.dashboardReducer.getUser.loading)
  ...
  console.log("userData of user }}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}", userData)
                  ...
                  <div className="flex items-center">
                    <div className="flex items-center">
                      ...
                      <div className="md:max-w-[130px] max-w-[130px]">
                        <span className="block text-sm text-gray-900 truncate font-medium ms-2">{userData?.name}</span>
                        <span className="block text-xs text-gray-500 truncate ms-2">{userData?.email}</span>
                      </div>
                    </div>
                ...
                {isDropdownOpen && (
                  <div className="absolute z-50 top-24 right-4  sm:top-12 my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-md" ref={dropdownRef}>
                    <div className="px-4 py-3" onClick={() => closeDropdown()} >
                      <span className="block text-sm text-gray-900 truncate">{userData?.name}</span>
                      <span className="block text-xs text-gray-500 truncate">{userData?.email}</span>
                    </div>
