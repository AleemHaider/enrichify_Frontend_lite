...
import { generateApiKey, getApiKeys } from "../../redux/projects/action"; // assuming you have these actions
import { useNavigate, useParams } from "react-router-dom";
import DataTable from "react-data-table-component";

const ApiKeyModal = () => {
  const { key } = useParams();
  const [username, setUsername] = useState(""); // For generating API key
  ...
  const handleGenerateApiKey = () => {
    if (username !== "") {
      dispatch(generateApiKey({ secret_key: key, api_key_name: username }));
          ...
          <div className="relative p-4 max-w-4xl w-full bg-white rounded-lg shadow">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {step === 1 ? "API Keys" : "Generate API Key"}
              </h3>
              <button
                onClick={toggleModal}
            ...
            {step === 1 ? (
              // Step 1: Display DataTable or go directly to Step 2 if no data exists
              <div>
                {apiKeys?.length > 0 ? (
                  ...
                  <div>
                    <p>No API keys found. Please generate a new API key.</p>
                    <div className="flex justify-end mt-4">
                      <button
                        onClick={() => setStep(2)}
                        className="bg-blue-700 hover:bg-blue-800 text-white font-medium py-2 px-4 rounded-lg"
